...
For `central_line` and `staph`, 37.8% and 44.2% of their points have a value of 0%. These points may skew my overall results, but since they make up a large proportion of the values, I did not remove them entirely. Instead, I only removed them temporarily in some exploratory graphs for clarity.
...
- 3 pairs of choropleth maps showing the means of `staph` and `stream`, `covid_vac` vs `clot`, and `flu_vac` vs `hem`. I selected these pairs since according to the previous heatmap, they have the largest correlations of any pair of predictors and outcomes. (This is explained in the next section.) I first obtained a dataset with the coordinates of state boundaries from the maps package, to which I had to add abbreviations for each state. I then merged it with my main dataset and plotted it using `geom_polygon` from ggplot2;
- 3 boxplots showing the distributions of the outcomes grouped by ED volume. I used `geom_boxplot` from ggplot2 and `plot_layout` from the patchwork package;
- 3 scatterplots showing the relations between the log-transformed outcomes vs `flu_vac`, `covid_vac`, and `staph`, grouping by ED volume. I transformed the outcomes since I suspected heteroskedasticity in their values. As with the choropleth maps, I selected these pairs of variables since they have the largest correlations of any pair of predictors and outcomes in the heatmap. I used `geom_point` and `geom_smooth` from ggplot2 and `plot_layout` from the patchwork package;
- Summaries of ANOVA F-tests (not shown) for all predictors and outcomes. I used `aov` from the stats package and `kable_styling` from kableExtra;
- 3 tables showing summaries of regression models for the following formulae: `hem ~ flu_vac`, `clot ~ central_line + staph + covid_vac`, and `stream ~ central_line + staph`. For each dependent variable, I selected these predictors since they have the most significant P-values in the ANOVA analysis with the same dependent variable. I used `lm` from the stats package and `kable_styling` from kableExtra.
...
kable(merged_summary, caption = "Summary of the numerical variables (rates)") |> 
...
ggplot(merged, aes(x = stream)) +
  ...
  plot_layout(ncol = 3) +
  plot_annotation(title = "Distributions of the outcome variables")
```
...
As such, the outcomes are not transformed for the linear models, which we now turn our attention to.

## Regression analysis

The ANOVA test for `hem ~ central_line + staph + ed_vol + covid_vac + flu_vac` shows the most significant predictor as `flu_vac`, which has a P-value of 1.1e-6 < 0.01. As such, it will be the only predictor for the model for `hem`.

```{r, echo = FALSE}
# kable(as.data.frame(summary(aov(hem ~ central_line + staph + ed_vol + covid_vac + flu_vac, merged))[[1]]), caption = "ANOVA test for $hem \\sim central\\_line + staph + ed\\_vol + covid\\_vac + flu\\_vac$")

# summary(lm(hem ~ flu_vac, merged))

coef1 <- coef(summary(lm(hem ~ flu_vac, merged)))
colnames(coef1) <- c("Estimate", "Std. error", "t-value", "P-value")
kable(coef1, caption = "Regression summary for $hem$ vs $flu\\_vac$")
```

In table 2, the summary for the model `hem ~ flu_vac` shows that for every increase of 1% in the rate of staff vaccinated against the flu, the rate of postoperative hemorrhage/hematoma increases by 0.00296%. The corresponding P-value is 0 < 0.01, indicating that the predictor is significant. However, the adjusted R-squared value (not shown) is 0.01301, indicating that the model explains very little of the variation in the data.

The ANOVA test for `clot ~ central_line + staph + ed_vol + covid_vac + flu_vac` shows the most significant predictors as `central_line`, `staph`, and `covid_vac`, which have P-values of 3.3e-4 < 0.01, 6.2e-4 < 0.01, and 3e-7 < 0.01 respectively. As such, they will be the predictors for the model for `clot`.

```{r, echo = FALSE}
# kable(as.data.frame(summary(aov(clot ~ central_line + staph + ed_vol + covid_vac + flu_vac, merged))[[1]]), caption = "ANOVA test for $clot \\sim central\\_line + staph + ed\\_vol + covid\\_vac + flu\\_vac$")
...
kable(coef2, caption = "Regression summary for $clot$ vs $central\\_line$, $staph$, and $covid\\_vac$")
```

In table 3, the summary for the model `clot ~ central_line + staph + covid_vac` shows that:
...
The ANOVA test `stream ~ central_line + staph + ed_vol + covid_vac + flu_vac` shows the most significant predictors as `central_line` and `staph`, which have P-values of 6.5e-4 < 0.01 and 1e-5 < 0.01 respectively. As such, they will be the only predictors for the model for `clot`.

```{r, echo = FALSE}
# kable(as.data.frame(summary(aov(stream ~ central_line + staph + ed_vol + covid_vac + flu_vac, merged))[[1]]), caption = "ANOVA test for $stream \\sim central\\_line + staph + ed\\_vol + covid\\_vac + flu\\_vac$")
...
kable(coef3, caption = "Regression summary for $clot$ vs $central\\_line$ and $staph$")
```

In table 4, the summary for the model `stream ~ central_line + staph` shows that:
...
var_imp <- function(model, outcome_name, method_name) {
  imp <- as.data.frame(varImpPlot(model))
  imp$var_names <- rownames(imp)
  imp |> ggplot(aes(x = reorder(var_names, IncNodePurity),
                    y = IncNodePurity)) +
    geom_point() +
    geom_segment(aes(x = var_names, xend = var_names,
                     y = 0, yend = IncNodePurity)) +
    labs(title = paste("Variable importance for", outcome_name,
                       "using", method_name),
         x = "Variable", y = "Increase in node purity") +
    coord_flip()
}

var_imp_xgb <- function(model, outcome_name) {
...
# var_imp(bag1, "hem", "bagging")
# var_imp(bag2, "clot", "bagging")
# var_imp(bag3, "stream", "bagging")
```
...
# var_imp(rf1, "hem", "random forest")
# var_imp(rf2, "clot", "random forest")
# var_imp(rf3, "stream", "random forest")
```
...
# var_imp_xgb(xgb1, "hem")
# var_imp_xgb(xgb2, "clot")
# var_imp_xgb(xgb3, "stream")
```
...
data.frame(Method = c("Bagging", "Random forest", "XGBoost"), 
  ...
  labs(title = "Test MSE for each method for hem",
       x = "Method", y = "MSE") +
  coord_flip()
...
data.frame(Method = c("Bagging", "Random forest", "XGBoost"), 
  ...
  geom_point() +
  geom_segment(aes(x = Method, xend = Method, y = 0, yend = MSE)) +
  labs(title = "Test MSE for each method for clot",
       x = "Method", y = "MSE") +
  coord_flip()
...
data.frame(Method = c("Bagging", "Random forest", "XGBoost"), 
  ...
  geom_point() +
  geom_segment(aes(x = Method, xend = Method, y = 0, yend = MSE)) +
  labs(title = "Test MSE for each method for stream",
       x = "Method", y = "MSE") +
  coord_flip()
```
